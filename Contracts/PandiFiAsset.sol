...
event TransferSingle(address indexed operator, address from, address indexed to, uint256 indexed assetId, address value);
...
event Approval(address indexed _owner, address indexed _spender, uint256 indexed _id, uint256 _oldValue, uint256 _value);
    ...
    //assetId=> servicing data
    mapping(uint256 =>bytes32) private servicingData;
...
/////////////////////////////////////////// ERC1155 //////////////////////////////////////////////
        function getOwnedIds() view external returns(uint256[] memory){
        uint256[] memory owned = new uint256[](nonce);
        for (uint i = 0; i< nonce; i++){
            if(balances[i][msg.sender]>0){
                owned[i] = i;
            }
        }
        return owned;
    ...
    // Batch create tokens. Assign directly to _to[].
    function createBatch(address[] calldata _to, uint256[] calldata _quantities) external returns(uint256 _id) {
        _id = nonce;
    ...
    */
    ...
    /**
    At present servicing data can only be added by the owner of the contract.  However, in the future
    we can add a mapping for approved servicing data providers. 
     */
    function setServicingData(uint256 _assetId, bytes32 _data) external returns(bool){
        require(balances[_assetId][msg.sender]>0, "You do not have data rights");
        
    }
    
    /*
    * At present servicing data is ONLY accessible by loan owner.  Eventually, this would need to change
    * but we will maintain this standard for now
    */

    function getServicingData(uint256 assetId) view external returns (bytes32){
        require(balances[assetId][msg.sender]>0, "Msg.sender does not have permission");
        return servicingData[assetId];
    }
    ...
    */
    function safeTransferFrom(address _from, address _to, uint256 _id, uint256 _value, bytes calldata _data) external override/*payable*/  {
        if (msg.sender != _from && operatorApproval[_from][msg.sender]!=true) {
            require(allowances[_from][msg.sender][_id]>=_value, "You don't have permission to spend that much");
            _safeTransferFrom(_from, _to, _id, _value, _data);
            allowances[_from][msg.sender][_id] = allowances[_from][msg.sender][_id].sub(_value);
        }
        else if(msg.sender!= _from && operatorApproval[_from][msg.sender]==true){
            _safeTransferFrom(_from, _to, _id, _value, _data);
            operatorApproval[_from][msg.sender]=false;
        }
        else{
            _safeTransferFrom(_from, _to, _id, _value, _data);
        }
    ...
    */
    function setApprovalForAll(address _operator, bool _approved) external override returns(bool){
